...
pronombre('ello', 'it', 'tercera', 'singular').
...
pronombre('cuánto', 'how_much', 'tercera', 'singular').
pronombre('cuánta', 'how_much', 'tercera', 'singular').
pronombre('cuántos', 'how_many', 'tercera', 'plural').
pronombre('cuántas', 'how_many', 'tercera', 'plural').
...
% === REGLAS DE ACCESO GENERAL CON N�MERO ===
traducir(PalabraEsp, PalabraEng, Numero) :-
    sustantivo(PalabraEsp, PalabraEng, Numero).
traducir(PalabraEsp, PalabraEng, _) :-
    verbo(PalabraEsp, _, PalabraEsp, PalabraEng). % Para verbos conjugados
traducir(PalabraEsp, PalabraEng, _) :-
    adjetivo(PalabraEsp, PalabraEng).
traducir(PalabraEsp, PalabraEng, Numero) :-
    determinante(PalabraEsp, PalabraEng, Numero, _).
traducir(PalabraEsp, PalabraEng, Numero) :-
    pronombre(PalabraEsp, PalabraEng, _, Numero).
traducir(PalabraEsp, PalabraEng, _) :-
    preposicion(PalabraEsp, PalabraEng).
traducir(PalabraEsp, PalabraEng, _) :-
    adverbio(PalabraEsp, PalabraEng).
traducir(PalabraEsp, PalabraEng, _) :-
    conjuncion(PalabraEsp, PalabraEng).

% Para verbos en infinitivo
traducir_infinitivo(InfinitivoEsp, InfinitivoEng) :-
    verbo(InfinitivoEsp, InfinitivoEng, _, _).

% Para verbos conjugados en 3ra persona
traducir_verbo_conjugado(ConjugadoEsp, ConjugadoEng) :-
    verbo(_, _, ConjugadoEsp, ConjugadoEng).
